#!/bin/bash
#
# Run get_emop_time.sh in Docker
# Before running this script, run `docker login`
# Usage: get_emop_time_in_docker.sh <projects-list> <sha-dir> <output-dir> [timeout=86400s] [profiling: true/false]
...
function check_input() {
  if [[ ! -d ${SHA_DIR} || ! -f ${PROJECTS_LIST} || -z ${OUTPUT_DIR} ]]; then
    echo "Usage: get_emop_time_in_docker.sh <projects-list> <sha-dir> <output-dir> [timeout=86400s]"
    exit 1
  fi
  ...
  OUTPUT_DIR=${OUTPUT_DIR}/get_emop_time
