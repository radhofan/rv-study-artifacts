#!/bin/bash
#
# Run profiler in Docker
# Before running this script, run `docker login` first.
# Usage: ./profile_in_docker.sh <projects-list> <output-dir> [timeout=86400] [branch]
#
PROJECTS_LIST=$1
OUTPUT_DIR=$2
TIMEOUT=$3
...
function check_input() {
  if [[ ! -f ${PROJECTS_LIST} || -z ${OUTPUT_DIR} ]]; then
    echo "Usage: profile_in_docker.sh <projects-list> <output-dir>"
    exit 1
  fi
...
function run_project() {
  ...
  timeout ${TIMEOUT} docker exec -w /home/tsm/tsm-rv -e M2_HOME=/home/tsm/apache-maven -e MAVEN_HOME=/home/tsm/apache-maven -e CLASSPATH=/home/tsm/aspectj-1.9.7/lib/aspectjtools.jar:/home/tsm/aspectj-1.9.7/lib/aspectjrt.jar:/home/tsm/aspectj-1.9.7/lib/aspectjweaver.jar: -e PATH=/home/tsm/apache-maven/bin:/usr/lib/jvm/java-8-openjdk/bin:/home/tsm/aspectj-1.9.7/bin:/home/tsm/aspectj-1.9.7/lib/aspectjweaver.jar:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin ${id} timeout ${TIMEOUT} bash experiments/profile_with_mop.sh ${repo} ${sha} /home/tsm/logs /home/tsm/async-profiler-2.9-linux-x64/build/libasyncProfiler.so &> ${OUTPUT_DIR}/${project_name}/docker.log
